  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <UserAvatar profilePicture={user.image} />
...
function SelectedUserTag({ user, onRemove }: SelectedUserTagProps) {
  return (
    ...
    >
      <UserAvatar profilePicture={user.image} size={24} />
      <p className="font-bold">{user.name}</p>
...
export default function NewChatDialog ({
    ...
    return <Dialog open onOpenChange={onOpenChange}>
        <DialogContent className="bg-card p-0">
            ...
            <div>
                <div className="group relative">
                    <SearchIcon className="absolute left-5 top-1/2 size-5 -translate-1/2 transform text-muted-foreground group-focus-within:text-primary"/>
                    <input
                        placeholder="Search users..."
                        className="h-12 w-full pe-4 ps-14 focus:outline-none"
                        value={searchInput}
                ...
                {!!selectedUsers.length && (
                    <div className="mt-4 flex-wrap gap-2 p-2">
                        {selectedUsers.map(user => (
                            <SelectedUserTag
                                key={user.id}
                                user={user}
